...
**TL;DR:** In this post I describe how we used the vLLM inference engine to classify 35k videos collected from TikTok.
I share Python code for video classification using the vLLM library in @sec-python-code, 
as well as the SLURM file for our workload in @sec-slurm-file.

# Motivation

For the research project I collaborated with my colleague at LMU Munich, [Renata Topinkova](https://renatatopinkova.github.io/), a computational social sciences researcher. 
...
This post focuses mainly on the **large inference workload**, which required running a large multimodal LLM (72B parameters) on 35k videos as quickly as possible before the deadline.
...
On top of that, the cues we were looking in the videos were often subtle and implicit: E.g.,
...
The only shortcoming of this model is that it does not process audio natively.
...
Some models that are capable of processing audio natively have been released recently like Qwen2.5-Omni or Phi-4 [@xu2025qwen25omnitechnicalreport;@abdin2024phi4technicalreport].
...
Neverthelss, they are a very promising next step to explore.
...
On the LRZ AI Systems, one cannot install and customize a Python environment on the worker nodes directly.
...
The code in @sec-slurm-file shows how to use these commands in a SLURM file.
...
The code in @sec-slurm-file shows how `SLURM_ARRAY_TASK_ID` is used in a SLURM file.
...
To cope with this, the server evicts blocks from the KV cache, and puts most requests back in the queue, where they effectively sit idle.
...
Also, we delete the created enroot container after the job is finished.
...
I hope this post is useful to other researchers looking to analyze large multimodal/video datasets with LLMs that require multiple GPUs.

In the future I hope to scale this pipeline to 1M videos, and push the throughput limits with quantization methods, speculative decoding, and test audio-capable models.
If you are interested in collaborating on large-scale inference, reach out to me! 🚀

# SLURM File {#sec-slurm-file}
Below is a Github gist of the SLURM file we used to submit our workload on the LRZ AI Systems.
