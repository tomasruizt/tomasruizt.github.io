...
An important shortcoming of this model is that it does not process audio natively.
...
In terms of the inference workload, we processed over 552 million input tokens, and generated over 9 million output tokens.
Per request, we processed 15.8k input tokens and produced over 250 output tokens, on average.
That is a ratio of 61 input tokens per output token, much in contrast to other workloads like solving mathematical problems that often have the reverse ratio.
In @fig-input-tokens and @fig-output-tokens shows the distribution of input and output tokens for the entire workload.
...
Dividing the ~15k input tokens and ~60 output tokens per request, give a throughput of 5k input tokens and 20 output tokens per second per model instance.
In terms of total time, the workload required 3s/post * 35k posts = 105k s $\approx$ 29hr.
...
In the future I hope to scale this pipeline to 1M videos, and push the throughput limits with quantization methods and speculative decoding.
