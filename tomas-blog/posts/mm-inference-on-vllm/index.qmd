...
I share lessons learned about computing on SLURM, about parallelism strategies (tensor- and data-parallelism), and share practical tips for fault tolerance and for video inference with vLLM.
...
**Video Input:** Feeding a video to model through vLLM requires a specific set up:
...
This removes the serialization and de-serialization overhead.
...
In terms of social science, we found interesting results, e.g. that a lot of posts promoted a product (42%), and that the most common product sold were dietary supplements (16%).
...
On this subset, we also used Gemini-2.5-Pro for classification, intended as upper performance reference.
We compare both models in terms of precision, recall, and F1 scores in @tbl-model-comparison. 
...
Its possible to combine the few human labels with the thousands of structurally biased AI labels to make this estimate.
...
Our workload was a prefill-heavy workload, and I showed how our achieved throughput is competitive with existing benchmarks by NVIDIA and vLLM.
Finally, I explained how to use thousands of potentially biased AI labels to make statitically valid arguments.
I hope this post is useful to other researchers aiming to analyze large video datasets with multimodal LLMs that require multiple GPUs.
