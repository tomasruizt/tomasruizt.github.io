...
Using the grouped blocks as suggested by the Triton docs improved the performance a lot, increasing L2 cache throughput from 50% to 90%.

Finally, the profiler shows bank conflicts that occur when saving to the shared memory.
This is relatively easy to fix in CUDA by paddin the shared memory array with +1, since
we directly control shared memory size.
