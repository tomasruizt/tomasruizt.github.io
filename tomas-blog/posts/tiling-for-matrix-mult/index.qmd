...
**Summary:** This post is about the concept of **tiling**, which is a technique used to reduce the number of memory accesses performed during matrix multiplication. We will see why this improves compute intensity and speeds up the matrix multiplication operation, not only on CPUs, but also in GPUs.
...
For simplicity, assume $A$ and $B$ are square matrices of size $n$. Below is a basic pseudocode for the operation, which involves approximately $n^3$ floating-point operations (flops), because of the triple nested loop:
...
## Naive Memory Access
In the naive approach, computing $C_{ij}$ requires fetching $n$ elements from $A$ (a row) and $n$ elements from $B$ (a column). That makes $2n$ memory accesses. Then we perform the dot product of the row and the column to get the element $C_{ij}$, which makes $2n$ flops. Thus, the compute intensity is:
$$
\text{Compute Intensity} = \frac{\text{flops}}{\text{memory transfers}} = \frac{2n}{2n} = 1
$$

Can we do better? In this naive implementation, we are doing redundant memory transfers: For example, to compute $C_{11}$, and $C_{12}$, we fetch the first row of $A$ twice from main memory.
