...
My benchmarks show that draft_model can achieve **speedups of up to 3.55×** on InstructCoder, outperforming EAGLE-3, while requiring no specialized training.
...
### Models Evaluated
I evaluated two model families: Qwen3 (dense) and LLaMa 3.3 70B.
A summary of the models evaluated is shown in @tbl-models.
For vanilla decoding, I evaluate the target model without any speculative decoding.
Within the Qwen3 family, using the 0.6B model as a draft_model raised a runtime exception (likely a bug).
The Qwen3 familiy was evaluated on a single NVIDIA H100 GPU, while the LlaMa 3.3 family required 2× H100 GPUs (tensor-parallelism).

| Family | Target Model | Draft Model(s) | EAGLE-3 Model | GPUs | 
|:--|:---|:---|:------|:-|
| **Qwen3** | Qwen3-32B^[<https://huggingface.co/Qwen/Qwen3-32B>] | Qwen3-1.7B^[<https://huggingface.co/Qwen/Qwen3-1.7B>] <br> Qwen3-4B^[<https://huggingface.co/Qwen/Qwen3-4B>] | Qwen3-32B-speculator.eagle3^[<https://huggingface.co/RedHatAI/Qwen3-32B-speculator.eagle3>] | 1× H100 96GB |
| **LLaMa-3.3** | Llama-3.3-70B-Instruct^[<https://huggingface.co/meta-llama/Llama-3.3-70B-Instruct>] | Llama-3.2-1B^[<https://huggingface.co/meta-llama/Llama-3.2-1B>] | EAGLE3-LLaMA3.3-Instruct-70B^[<https://huggingface.co/yuhuili/EAGLE3-LLaMA3.3-Instruct-70B>] | 2× H100 80GB | 

: Model configurations evaluated {#tbl-models}
...
This larger dataset enables testing at higher batch sizes (I benchmark up to 256^[On InstructCoder with LLaMa 70B the KV cache fills up completely at batch size 64, meaning some requests are put to wait. Increasing the batch size beyond doesn't increase the parallelism, so I only benchmark up to 64 in this case.]).
...
All benchmark results are available in a single [Parquet file](https://github.com/tomasruizt/vllm-scripts/blob/d94d714fa4e451591f628542a57bbc7f9c2d2619/results/full-bench-data.parquet) for download.


I benchmarked performance using `vllm bench serve` to inference metrics: token throughput (toks/s), TTFT, TPOT, and ITL.
The **speedup ratio** metric is computed as the token throughput of speculative decoding over vanilla decoding.
...
I ran benchmarks across different batch sizes, since speculative decoding is known to be most effective in lower batch sizes.
The batch size is implemented as concurrent requests sent to the server (`--max-concurrency`).
I set `--request-rate` equal to `--max-concurrency` to prevent all prefills from happening at the exact same time, which would inflate the TTFT for vanilla decoding.
...
#### Hardware
The Qwen3 experiments were run on an NVIDIA H100 96GB GPU, within the SLURM cluster of the [LRZ AI Systems](https://doku.lrz.de/lrz-ai-systems-11484278.html),
while the LLaMa 3.3 experiments were run on a 2× H100 80GB GPU launched from [NVIDIA Brev](https://developer.nvidia.com/brev).

## Results

### Versus Vanilla Decoding
I compare the token throughput of speculative decoding (draft_model) vs. vanilla decoding on @fig-total-token-throughput-short.
We observe that using speculative decoding **dramatically increases** token throughput compared to vanilla decoding.
The number of speculative tokens $K$ was chosen to maximize the speedup ratio.
In the next section, we see what happens when we change $K$.

::: {.panel-tabset group="dataset"}
# InstructCoder (LLaMa 70B)
![Token Throughput Metrics.](imgs/llama70b/InstructCoder/total_token_throughput_short.png){width="60%" text-align="left" fig-alt="Token throughput metrics" #fig-total-token-throughput-short}

# MT-Bench (Qwen3)
![Token Throughput Metrics.](imgs/qwen/mt-bench/total_token_throughput_short.png){width="60%" text-align="left" fig-alt="Token throughput metrics"}

# InstructCoder (Qwen3)
![Token Throughput Metrics.](imgs/qwen/InstructCoder/total_token_throughput_short.png){width="60%" text-align="left" fig-alt="Token throughput metrics"}
:::
...
To highlight this dynamic, I compared speedup ratios across different values of $K$.
...
::: {.panel-tabset group="dataset"}
# InstructCoder (LLaMa 70B)
![Throughput as a function of $K$, evaluating both the 1.7 and the 4B models as draft models.](imgs/llama70b/InstructCoder/draft_model_ratios.png){width="60%" #fig-draft-model-ratios}

# MT-Bench (Qwen3)
![Throughput as a function of $K$, evaluating both the 1.7 and the 4B models as draft models.](imgs/qwen/mt-bench/draft_model_ratios.png)

# InstructCoder (Qwen3)
![Throughput as a function of $K$, evaluating both the 1.7 and the 4B models as draft models.](imgs/qwen/InstructCoder/draft_model_ratios.png)
:::

The results show several key patterns:

- **Multi-GPU Setting**: The greatest speedups (3.55×) were achieved on the multi-GPU setting (LLaMa 70B).
This was a surprise, because I thought that the intra-GPU communication overhead would slow down the draft model significantly.
While this effect might still be present, it does not dominate performance.
This shows that draft_model is compatible and effective with tensor parallelism > 1.

- **Batch Size Dependency**: Speedup is maximal for small batch sizes (concurrency 1-4) and decays for larger batch sizes.
...
- **Saturation Effect**: Increasing $K$ beyond a certain value does not consistently improve speedups, indicating a saturation point.
Saturation is reached around 3 or 4 for the Qwen3 family, and around 7 for the LLaMa 70B family.

- **Draft Model Size Trade-off**: Within the Qwen3 family, the smaller 1.7B model generally achieves higher speedups than the larger 4B model.
...
- **Low $K$ Robustness**:
In the Qwen3 family, the curve for $K=1$ is interesting.
It achieves only modest speedups at small batch sizes, but in large batch sizes, it is more robust and speedups don't decay as much as for higher $K$ values.
The effect is particularly visible in the InstructCoder dataset, with the 1.7B draft model.
...
The EAGLE-3 paper by @li2025eagle reports speedup ratios on SGLang^[<https://github.com/sgl-project/sglang>] between 1.38× and 1.82× at batch sizes ranging from 2 to 64 (table 3), and slightly lower speedups on vLLM (table 5).

My own benchmark results in @fig-eagle3-ratios show **greater speedups on vLLM**: peaking at 2.94× for the LLaMa 70B family and 2.10× for the Qwen3 family.
...
::: {.panel-tabset group="dataset"}
# InstructCoder (LLaMa 70B)
![Speedup ratios of EAGLE-3](imgs/llama70b/InstructCoder/eagle3_ratios.png){width="60%" text-align="left" fig-alt="Speedup ratios of EAGLE-3" #fig-eagle3-ratios}

# MT-Bench (Qwen3)
![Speedup ratios of EAGLE-3](imgs/qwen/mt-bench/eagle3_ratios.png){width="60%" text-align="left" fig-alt="Speedup ratios of EAGLE-3"}

# InstructCoder (Qwen3)
![Speedup ratios of EAGLE-3](imgs/qwen/InstructCoder/eagle3_ratios.png){width="60%" text-align="left" fig-alt="Speedup ratios of EAGLE-3"}
:::

@fig-draft-model-vs-eagle3-ratios compares the speedup ratios of both speculative decoding methods.
I used $K=4$ in the Qwen3 family and $K=7$ in the LLaMa 70B family.
The speedups of draft_model are surprisingly good, often exceeding those of EAGLE-3 by a significant margin.
This is particularly visible on InstructCoder (LLaMa 70B setting) and MT-Bench (Qwen3 setting).
Both methods have very similar speedups on InstructCoder (Qwen3 setting).

::: {.panel-tabset group="dataset"}
# InstructCoder (LLaMa 70B)
![Comparison of Speedups of Draft Model and EAGLE-3](imgs/llama70b/InstructCoder/draft_model_vs_eagle3_ratios.png){width="60%" text-align="left" fig-alt="Comparison of Speedups of Draft Model and EAGLE-3" #fig-draft-model-vs-eagle3-ratios}

# MT-Bench (Qwen3)
![Comparison of Speedups of Draft Model and EAGLE-3](imgs/qwen/mt-bench/draft_model_vs_eagle3_ratios.png){width="60%" text-align="left" fig-alt="Comparison of Speedups of Draft Model and EAGLE-3"}

# InstructCoder (Qwen3)
![Comparison of Speedups of Draft Model and EAGLE-3](imgs/qwen/InstructCoder/draft_model_vs_eagle3_ratios.png){width="60%" text-align="left" fig-alt="Comparison of Speedups of Draft Model and EAGLE-3"}
:::

Both EAGLE-3 and draft_model have very different **Acceptance Lengths** (AL), shown in @fig-acceptance-lengths-vs-num-spec-toks.
The AL curve for EAGLE-3 is flatter, meaning that predicting more tokens leads to diminishing returns.
In contrast, the AL curve for draft models continues to climb with more speculative tokens.
Despite the EAGLE-3 draft models being lighter, the lower AL is a limiting factor that prevents it from reaching the same speedups as draft_model.

::: {.panel-tabset group="dataset"}
# InstructCoder (LLaMa 70B)
![Acceptance Lengths for draft_model and EAGLE-3.](imgs/llama70b/InstructCoder/acceptance_length_vs_num_spec_toks.png){width="90%" text-align="left" fig-alt="Acceptance Lengths for draft_model and EAGLE-3" #fig-acceptance-lengths-vs-num-spec-toks}

# MT-Bench (Qwen3)
![Acceptance Lengths for draft_model and EAGLE-3.](imgs/qwen/mt-bench/acceptance_length_vs_num_spec_toks.png){width="90%" text-align="left" fig-alt="Acceptance Lengths for draft_model and EAGLE-3"}

# InstructCoder (Qwen3)
![Acceptance Lengths for draft_model and EAGLE-3.](imgs/qwen/InstructCoder/acceptance_length_vs_num_spec_toks.png){width="90%" text-align="left" fig-alt="Acceptance Lengths for draft_model and EAGLE-3"}
:::

#### Token Throughput

@fig-total-token-throughput-full shows the Output Token Throughput (toks/s) metric for both speculative decoding methods and vanilla decoding (SD=None).
...
::: {.panel-tabset group="dataset"}
# InstructCoder (LLaMa 70B)
![Token Throughputs of all Methods](imgs/llama70b/InstructCoder/total_token_throughput_full.png){width="60%" text-align="left" fig-alt="Token Throughputs of all Methods" #fig-total-token-throughput-full}

# MT-Bench (Qwen3)
![Token Throughputs of all Methods](imgs/qwen/mt-bench/total_token_throughput_full.png){width="60%" text-align="left" fig-alt="Token Throughputs of all Methods"}

# InstructCoder (Qwen3)
![Token Throughputs of all Methods](imgs/qwen/InstructCoder/total_token_throughput_full.png){width="60%" text-align="left" fig-alt="Token Throughputs of all Methods"}
:::
...
::: {.panel-tabset group="dataset"}
# InstructCoder (LLaMa 70B)
![Inference Metrics. 99th percentile of TPOT, ITL, and TTFT. Lower is better.](imgs/llama70b/InstructCoder/ttft_itl_tpot.png){width="100%" text-align="left" fig-alt="Inference Metrics" #fig-ttft-itl-tpot}

# MT-Bench (Qwen3)
![Inference Metrics. 99th percentile of TPOT, ITL, and TTFT. Lower is better.](imgs/qwen/mt-bench/ttft_itl_tpot.png){width="100%" text-align="left" fig-alt="Inference Metrics"}

# InstructCoder (Qwen3)
![Inference Metrics. 99th percentile of TPOT, ITL, and TTFT. Lower is better.](imgs/qwen/InstructCoder/ttft_itl_tpot.png){width="100%" text-align="left" fig-alt="Inference Metrics"}
:::
...
- **TTFT**: The TTFT is generally higher for speculative decoding methods than vanilla decoding because the server must prefill with both the draft and target models, rather than just the target model.
In MT-Bench (Qwen3), the TTFT of EAGLE-3 is consistently higher than for draft_model, which is surprising since EAGLE-3 drafters are lighter.
...
### Limitations
The draft model must share the same tokenizer as the target model, i.e., both models must "speak the same language."
This is typically the case when models belong to the same family.

At the moment, draft_model is limited to models with only a single KV-cache group.
There is an open issue about this on Github [(#33133)](https://github.com/vllm-project/vllm/issues/33133)

My benchmarks use the default `--output-len=200` parameter, which means that the speedups presented are valid for short-context inference.
Long context inference is more complex: performance is bottlenecked KV-cache loading, and batch sizes must become a lot smaller to fit in VRAM.
I might publish an analysis of long-context performance in the future.

### When to Use Draft Models vs. EAGLE-3

In theory, EAGLE-3 should achieve higher speedups than draft_model.
However, my benchmarks show that the higher acceptance lengths of draft_model dominate the speedups, making it a superior choice over EAGLE-3 in 2/3 tested settings.
...
- *Lower computational overhead*: EAGLE-3's lightweight draft models add minimal overhead, allowing the method to achieve high speedups (assuming good enough acceptance lengths).

My recommendation is to:

* Prefer using draft_model over EAGLE-3 when you have access to a smaller model from the same family and want to get started quickly without training a specialized draft model.
* If a corresponding EAGLE-3 drafter model already exists on Hugging Face, I still recommend comparing the performance of both methods on your workload, just to be sure.
* If you want to train your own EAGLE-3 drafter, you can use the vLLM speculators library^[<https://github.com/vllm-project/speculators>].
