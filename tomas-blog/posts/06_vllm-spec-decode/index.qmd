...
However, the next section will show that increasing concurrency further leads to a breakdown in throughput for speculative decoding.
...
<!-- draft_model achieves speedups between 1.39x and 2.0x for batch sizes up to 64, matching EAGLE-3 up to batch size 16.
...
We observe a saturation effect on the AL for EAGLE-3: The AL line starts to plateau around AL=2.5.
...
Beyond this point, the throughput of speculative decoding methods plateau, while vanilla decoding continues to scale with larger batch sizes.
...
All metrics generally increase (worsen) with batch size. We observe the following patterns
...
- **TPOT**: Both speculative decoding methods significantly reduce TPOT compared to vanilla decoding.
