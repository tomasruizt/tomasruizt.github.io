...
In this post, I benchmark the performance of this draft_model implementation and compare it against vanilla decoding and EAGLE-3, a modern speculative decoding method already supported by vLLM.
My benchmarks show that draft_model can achieve speedups of up to 2.24x on MT-Bench, outperforming EAGLE-3, while offering the advantage of requiring no specialized training.
...
This is most often a given when models belong to the same family.
...
It looks similar to @lst-set-forward-context, and its important to understand that it partially determines the inputs to the model.
...
mode** the model will dispatch to.
...
Its important to keep in mind that we deal with a distribution of points, and to visualize not only the mean but also the median and 99th percentile.
...
This meant that the server was batch processing all prefills at once in synchrony, and this inflated the TTFT for all requests of vanilla decoding.
...
To prevent this artifact, I'm from now on always setting a request rate (e.g. `--request-rate` equals `--max-concurrency`), so that the requests are sent on average over the first second.
...
### Code Evolution
The vLLM project is very short on reviewers and many people wish to contribute to it, which means that many PRs sit unreviewed for a long time.
My [PR](https://github.com/vllm-project/vllm/pull/24322) was open from 2025.09.05 and merged on 2026.01.19, 
which is 136 days or about 4.5 months (!), and that was despite relatively engaged reviewers.
That is a very long time to have a PR open, and the vLLM codebase evolves very quickly.
As a result, I had to resolve **merge conflicts** at least 10 times.
After resolving several very painful conflicts, I learned that some code changes are more likely to cause conflicts than others.
E.g. moving a function to a new file _and_ changing its body will create a painful merge conflict, because the function will evolve on main in its old location.
The same applies to extracting a superclass or child class, changing its body, and then moving it to a new file.
We should not wish for our PRs to stay open for a long time, but we should be prepared if the code evolves underneath our PR.
E.g. We can modify functions and classes in-place (same file), and defer moving functions/classes to other files in a subsequent PR.
This might sound strange for experienced engineers, but it is what I found effective to reduce the merge conflict pains.
