...
- **Draft Model Size Trade-off**: The smaller 1.7B draft model achieves higher speedups than the 4B, despite the 4B having better acceptance rates (@fig-acceptance-lengths-vs-num-spec-toks). The 4B's slower inference limits its overall speedup.
...
The shaded area represents (min, max) ALs over different batch sizes^[I had not expected AL variation with batch size. Perhaps batch invariance would solve this.].
...
My benchmarks show that draft_model outperforms EAGLE-3 in 2 of 3 tested settings, primarily due to higher acceptance lengths (@fig-acceptance-lengths-vs-num-spec-toks).
**Prefer draft_model** when a smaller model from the same family is available, no training required.
Distilled models (trained to mimic the larger model's logits) work especially well.
**Consider EAGLE-3** when a pre-trained EAGLE-3 drafter exists on Hugging Face and you want the lowest possible overhead.
When in doubt, benchmark both methods on your workload.
You can also train your own using the vLLM speculators library^[<https://github.com/vllm-project/speculators>].
...
The language is easy to learn and it gets you 85% of the way to speed-of-light performance.
...
It delivers substantial speedups over vanilla decoding (up to 3.55×) without specialized training.
Speedups are most pronounced at smaller batch sizes and fade at larger batch sizes.
The performance numbers are meant as a reference as of today (2026-01-28, commit `e1a34c3`), and can vary with model, workload, and deployment.
