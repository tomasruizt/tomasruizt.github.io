...
subtitle: "Benchmarks and Lessons Learned"
author:
  - name: Tomas Ruiz
    email: t.ruiz@lmu.de
...
In this post, I benchmark the performance of this draft_model implementation and compare it against vanilla decoding and EAGLE-3, a modern speculative decoding technique supported by vLLM.
My benchmarks show that draft_model can achieve speedups of up to 2.24x on MT-Bench, outperforming EAGLE-3, while requiring no specialized training.
At the end, I share lessons learned from working in the vLLM codebase and a few practical tips for approaching and navigating it.
...
The focus here is on benchmarking the new code and sharing lessons learned.
:::
...
I also recommend passing `--max-model-len` to leave more memory for the KV cache.
...
A speedup of 2x means speculative decoding finishes the workload in half the time of vanilla decoding.
...
In this section, I begin by sharing some quirks I learned about vLLM while implementing speculative decoding, and then in @sec-start-high-level I share tips about how to approach the vLLM codebase as a new contributor.
...
The forward context also includes the `batch_descriptor` and `cudagraph_runtime_mode` objects, which together determine which **CUDA graph mode** the model will dispatch to.
...
### CPU-GPU Synchronizations & Triton Kernels
I wrote about CPU-GPU synchronizations in full length in a [separate post](https://tomasruizt.github.io/posts/08_cpu_gpu_synchronization/).
In short: To write fast and efficient PyTorch code, its important to avoid CPU-GPU synchronizations.
Synchronizations prevent the CPU from scheduling enough work for the GPU to keep it busy, leading to low GPU utilization.
In the vLLM speculative decoding PR, I needed to prepare the input tokens for the draft_model, which required multiple PyTorch operations (including a synchronization).
To prevent CPU-GPU synchronizations, and also fuse all these operations into a single one, I implemented a custom [Triton kernel](https://github.com/vllm-project/vllm/blob/1209b784f2ba976eff2ea24bc33c61f35c6eb213/vllm/v1/spec_decode/draft_model.py#L234).
The kernel is quite simple, but it replaced around 8 calls to PyTorch functions. 
...
The average TTFT of the SD methods were consistently lower than for vanilla decoding.
...
![Statistical Artifact in TTFT between SD methods and vanilla decoding.](imgs/statistical-artifact.png){width="60%" text-align="left" fig-alt="Statistical Artifact in TTFT" #fig-statistical-artifact}

It's important to keep in mind that we deal with a distribution of points, and to visualize not only the mean but also the median and 99th percentile.
The mean can be heavily skewed by outliers, which happens for TTFT when requests are slow.
...
My [PR](https://github.com/vllm-project/vllm/pull/24322) was open from 2025.09.05 and merged on 2026.01.19—136 days, or about 4.5 months—despite having relatively engaged reviewers.
...
For example, moving a function to a new file _and_ changing its body creates painful merge conflicts, because the function continues to evolve on main in its old location.
The same applies to extracting a superclass or child class, changing its body, and then moving it to a new file.

While we shouldn't plan for PRs to stay open this long, we should be prepared for code to evolve underneath them.
...
This might sound counterintuitive to experienced engineers, but I found it effective for reducing merge conflict pain.
