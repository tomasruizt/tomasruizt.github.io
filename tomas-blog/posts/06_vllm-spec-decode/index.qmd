...
#### Follow-up Analysis: Full CUDA Graphs
Currently, vLLM supports two different CUDA graph modes: `piecewise` and `full`. 
The `full` CUDA graph mode achieves the highest speeds, but is not compatible with all features.
At the moment, draft models (both EAGLE-3 and draft_model) use the `piecewise` CUDA graph mode.
In this section, I estimate the speedups we could achieve by supporting the `full` CUDA graph mode for draft models.

1. **Speed of full CUDA graphs:**
I deactivate speculative decoding, run each model (Qwen3 32B and 1.7B) separately on a single request of the InstructCoder dataset with 1000 output tokens, and measure their respective ITLs.
This sets a baseline for the runtime to produce a single token under full CUDA graphs.

2. **Speed of piecewise CUDA graphs:**
I activate speculative decoding (draft_model), run draft + target, and measure the ITL.
I vary the number of speculative tokens to estimate the duration of each additional forward run of the draft model.
The resulting ITL regression is shown in @fig-itl-vs-num-spec-toks.
The intercept is the runtime of the target model within the SD framework, and the slope is the runtime of the draft model within the SD framework. 
...
Having the runtime of both models under both CUDA graph modes, we can calculate the speedups in @tbl-speed-comparison-full-vs-piecewise.
The column "SD Runtime" is the estimate based on the ITL regression mentioned above.
The column "Standalone Runtime" is the median ITL metric reported by `vllm bench serve` without speculative decoding.
The runtime of the draft model is 9.10% shorter under full CUDA graphs.
...
Now assuming we implemented the shorter runtime using full CUDA graphs, what end-to-end speedups for draft_model could we achieve?
I used the formula below to calculate the TPOT values, and forecast the speedups for full CUDA graphs.
$$
...
When I started this analysis, I expected larger improvements (around 20%).
Nevertheless, these forecasted improvements are significant, because they would be universal across all draft_model workloads.
