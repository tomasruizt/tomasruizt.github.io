...
The full source code is at [github.com/tomasruizt/fused-mm-sample](https://github.com/tomasruizt/fused-mm-sample).
All benchmarks run on [Modal](https://modal.com) cloud GPUs and are orchestrated via `make` targets.
...
The kernel benchmarks run a three-step pipeline: (1) execute on Modal, (2) download results, (3) generate plots.

```bash
make modal-triton-benchmark GPU=h100!         # single GPU
make modal-triton-benchmark-all-gpus          # all GPUs (B300, B200, H200, H100)
```

Results are saved to `benchmarking/modal-results/triton-bench-{GPU}/` with CSVs and plots.

### End-to-End vLLM Benchmarks

The vLLM benchmarks use the FMMS-integrated [vLLM fork](https://github.com/tomasruizt/vllm/tree/feature/fmms-sampler) and sweep across concurrency levels with 5 runs per configuration.
...
Results are saved to `benchmarking/modal-results/vllm-bench-{GPU}/` with per-model subdirectories.

### Unit Tests

The correctness tests verify the sampling distribution via a chi-squared goodness-of-fit test.
