...
- Expand the Correctness section: explain that the kernel output is stochastic, so a simple equality unit test is insufficient; instead the sampling distribution must match the reference. For the chi-squared test, mention the concrete parameters (10,000 samples, p<0.01 threshold, bins with expected count <5 excluded). For GSM8K, note that the baseline also samples (temperature=0.6, as set by Qwen3's chat template), so even reruns of the baseline give slightly different accuracy. This is why a paired bootstrap test (p=0.776) is used to compare FMMS vs baseline, rather than a raw accuracy comparison.
- Make all tables collapsible.
- Rename in the plots "Naive PyTorch Compiled" to "PyTorch Compiled Sampling".
- In "Relative Performance Across GPUs", put together the tabsets for both FlashInfer baselines.
-->
...
Across popular LLMs, vocab sizes cluster around 128K-152K, so the real variable is the hidden dimension `d`.
...
- **Small** (V=128,256, d=4,096): Llama 3 8B, Qwen3-8B, and notably Qwen3-235B MoE (which has d=4,096 despite 235B total parameters).
...
At N=128+, the baselines catch up as the matmul dominates and cuBLAS efficiency matters more.

::::: {.panel-tabset group="workload-size"}
# Small (d=4,096)

::: {.panel-tabset group="gpu-scaling-small"}
## B300
![Batch scaling on B300 (V=128,256, d=4,096).](imgs/batch-scaling-small-b300.png){width="100%" #fig-batch-scaling}
...
::: {.panel-tabset group="gpu-scaling-large"}
## B300
...
::: {.panel-tabset group="gpu-pytorch-small"}
#### B300
...
| --- | - | - | - | - | - | - | - | - | - |
| B300 | 1.51 | 1.44 | 1.44 | 1.45 | 1.44 | 1.42 | 1.30 | 1.03 | 0.85 |
...
::: {.panel-tabset group="gpu-pytorch-large"}
#### B300
...
FMMS is faster across all GPUs and all batch sizes from 1 to 64, with speedups of 19-52%.
The small config shows larger speedups (up to 1.52x) because the smaller hidden dimension makes the matmul more memory-bound, giving fusion a bigger advantage.
H100 retains its advantage at larger batch sizes than other GPUs: at N=128, it still achieves 1.17x (large) and 1.31x (small), while other GPUs drop to ~1.0.
...
::: {.panel-tabset group="gpu-flashinfer-small"}
#### B300
...
| GPU / Batch Size | 1 | 2 | 4 | 8 | 16 | 32 | 64 | 128 | 256 |
| --- | - | - | - | - | - | - | - | - | - |
| B300 | 1.68 | 1.78 | 2.16 | 1.98 | 2.12 | 2.01 | 1.78 | 1.21 | 1.12 |
...
::: {.panel-tabset group="gpu-flashinfer-large"}
#### B300
...
FMMS is up to **2.16x faster** (B300, small config, N=4).
Across typical decode batch sizes (1 to 64), FMMS is 14-116% faster.
B300 is a striking outlier: 1.5-2.2x speedups while B200/H200/H100 show 1.1-1.4x.
...
::: {.panel-tabset group="gpu-flashinfer2-small"}
#### B300
![Relative performance vs FlashInfer baselines on B300.](imgs/relative-perf-vs-flashinfer-small-b300.png){width="80%"}
...
| H100 | 1.18 | 1.17 | 1.15 | 1.14 | 1.14 | 1.12 | 1.08 | 0.92 | 0.73 |

### Large (d=8,192)

::: {.panel-tabset group="gpu-flashinfer2-large"}
#### B300
![Relative performance vs FlashInfer baselines on B300.](imgs/relative-perf-vs-flashinfer-large-b300.png){width="80%"}
...
A clear pattern emerges: **FMMS is faster at all batch sizes from 1 to 64 on every GPU tested**, with speedups of 2-18%.
It regresses at large batch sizes (N=128+).
...
# B300
![Roofline model on B300 (V=128,256, d=8,192). The ridge point marks the transition from memory-bound to compute-bound.](imgs/roofline-large-b300.png){width="85%" #fig-roofline}
...
::: {.panel-tabset group="gpu"}
# B300
![Memory throughput on B300 (V=128,256, d=8,192). The dashed line shows peak HBM bandwidth.](imgs/memory-throughput-large-b300.png){width="85%" #fig-memory-throughput}
