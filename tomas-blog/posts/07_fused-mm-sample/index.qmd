...
A clear pattern emerges: **FMMS is 3-15% faster at small batch sizes (N=1 to 64) but regresses at large batch sizes (N=128+)**. The crossover point varies by GPU, but is consistently in the N=64--128 range.
...
FMMS demonstrates that fusing matrix multiplication with sampling via the Gumbel-max trick is both correct and practical. The kernel is 3-15% faster than the best baselines at the batch sizes that matter most for LLM decode (N=1 to 64), where the operation is memory-bound and eliminating the extra HBM round-trip for logits directly improves throughput.
