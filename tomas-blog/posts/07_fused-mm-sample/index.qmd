...
- In "Scaling Behavior" the tabset labels say "Small (d=4,096)" and "Large (d=8,192)", suggesting only d differs. But V also changes (151,936 vs 128,256). Fix the labels to include both V and d.
- Reconcile "compute-bound at N=128" with actual per-GPU ridge points (153-295 depending on GPU).
- Mention ~90% peak memory bandwidth at low batch sizes (H200, H100). Explain what 100% would mean: load weights + hidden states from HBM, write back only the samples (a few ints) to GMEM.
- Make all tables collapsible.
- In the "Scaling Behavior" plot, document: left panel (time) lower is better, right panel (samples/ms) higher is better.
- In the memory throughput plot, put Speed-of-Light % on the left y-axis and throughput (GB/s) on the right y-axis, so the grid aligns with the SoL ticks.
...
- Expand the Correctness section: explain stochastic output, chi-squared test parameters (10,000 samples, p<0.01, bins with expected count <5 excluded), paired bootstrap test (p=0.776) for GSM8K.
- Makefiles are awesome. Document how I used them in this project.
...
A clear pattern emerges: **FMMS is faster at all batch sizes from 1 to 64 on every GPU tested**, with speedups of 3-25%.
