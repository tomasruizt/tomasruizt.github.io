...
2. **NVIDIA Nsight Systems Profiler**
3. **Deep dive: Dynamic tensor shapes**
...
The CPU is the boss of the GPU. CPU **schedules** PyTorch ops, which are queued and executed by the GPU.
...
for _ in range(n):
    ...
    out = fast_operation(out)  # e.g mean()
    if do_print:
...
Profiling Command:
``` shell
nsys profile -o profile.nsys-rep python cpu_gpu_sync_example.py --do-print
```
